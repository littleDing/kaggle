...
def feature_018(feature,groupby,cycle,c,kernel_base):
	...
	ans 	= pd.merge(IDS,mapping,how='left')
	ans.rename(columns={'Weekly_Sales':'Kernel18%s'%('_'.join(map(str,[groupby,cycle,c,kernel_base])))},inplace=True)
	return ans
...
def feature_019(feature,groupby,timeline,cycle,c,kernel_base):
	'''@return id => (kernel density)'''
	IDS 	= feature[['Store','Dept','Date','IsHoliday']]
	mapping = date_mapping_019('2010-01-01','2013-12-31',groupby,timeline,cycle,c,kernel_base)
	ans 	= pd.merge(IDS,mapping,how='left')
